...
| `hooks/useWindwosSize.ts` | 18 | window resize リスナー | ファイル名 typo（Windows → Windwos） |
...
| `useWindwosSize.ts` | ファイル名 | import パスが typo 依存 |
...
| 高 | Firebase Emulator Suite の導入 | オンライン機能をローカルで検証可能に |
...
| 中 | `useWindwosSize` → `useWindowSize` リネーム（import 一括更新） | 可読性向上。破壊的変更なし |
