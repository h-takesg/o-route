...
| キャンバス | Konva 9, react-konva 18, use-image | `/local` `/online` は `ssr: false`（ブラウザ専用） |
...
| 開発環境 | Dev Container（Ubuntu jammy, **Node 24 LTS**） | Active LTS（Krypton）。反映にはコンテナ再ビルドが必要 |
...
| 項目 | 内容 |
|------|------|
| テスト | ユニット 35 件（Vitest）+ E2E（Playwright）。初回は `npm run test:e2e:install` と `sudo npx playwright install-deps chromium` |
...
| Firebase 実環境 | Emulator + Playwright E2E でローカル検証可能（`npm run test:e2e`） |
...
| 済 | `canvas` 依存の削除 | `/local` `/online` を `ssr: false` にし npm `canvas` を除去 |
| — | CI（lint + build + test）の追加 | スキップ（手動で `npm run test` / `npm run build` を確認） |
...
| オンライン同期 | `OnlineCanvas.tsx`, `hooks/useOnlineLines.ts`, `hooks/useOnlineRoomImage.ts`, `database.rules.json`, `firebase.ts` |
